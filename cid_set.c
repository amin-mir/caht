...
size_t cid_iter_next_batch(struct cid_iter *iter, size_t batch_size,
						   uint64_t batch[batch_size]) {
	size_t i;
	for (i = 0; iter->idx < iter->len && i < batch_size; iter->idx++) {
		if (iter->ids[iter->idx] == EMPTY_VAL) {
			continue;
		}
		batch[i] = iter->ids[iter->idx];
		i++; 
	}
	return i;
}
